...
static int fts_read_and_report_foddata(struct fts_ts_data *data)
{
	...
	pr_info("FTS:read fod data: buf[2] = 0x%x\n", buf[2]);
	if (ret < 0) {
		...
		case 0x26:
			...
			if (buf[9] == 0) {
				...
				if (!data->fod_finger_skip) {
					...
					input_sync(data->input_dev);
					FTS_INFO("Report_0x152 DOWN for FingerPrint\n");
...
<<<<<<< HEAD
	...
	if (!ts_data->IRQ_skipped && spin_trylock(&i2c_resume_lock)) {
		/*FTS_ERROR("IRQ Thread get spinlock and go on\n");*/
...
#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	if(fts_finger_report_disable(ts_data, false) < 0)
