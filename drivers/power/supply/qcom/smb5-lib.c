					...
					union power_supply_propval *val)
{
	...
	if (val->intval == 1 && pval.intval >= 98) {
#ifdef CONFIG_BQ2597X_CHARGE_PUMP
...
static int smblib_therm_charging(struct smb_charger *chg)
{
	...
	if (chg->system_temp_level == 0) {
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3
#ifndef CONFIG_BQ2597X_CHARGE_PUMP
...
#ifdef CONFIG_BQ2597X_CHARGE_PUMP
#define SW_CV_DYNAMIC_FCC_MIN		1400000
#define SW_CV_DYNAMIC_FCC_MAX		3000000
int smblib_sw_cv_dynamic_fcc_limit(struct smb_charger *chg, int fcc_max)
{
	if (fcc_max <= SW_CV_DYNAMIC_FCC_MIN)
		chg->sw_cv_dynamic_fcc = SW_CV_DYNAMIC_FCC_MIN;
	else if (fcc_max >= SW_CV_DYNAMIC_FCC_MAX)
		chg->sw_cv_dynamic_fcc = SW_CV_DYNAMIC_FCC_MAX;
	else
		chg->sw_cv_dynamic_fcc = fcc_max;

	schedule_delayed_work(&chg->sw_cv_fcc_limit_work, 0);
	return 0;
}

static void sw_cv_fcc_limit_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
							sw_cv_fcc_limit_work.work);

	vote(chg->fcc_votable, MAIN_FCC_VOTER, true, chg->sw_cv_dynamic_fcc);
	if (chg->bms_psy) {

		if (chg->sw_cv_dynamic_fcc > SW_CV_DYNAMIC_FCC_MIN) {
			chg->sw_cv_dynamic_fcc -= 50000;
			pr_err("sw_cv_dynamic_fcc=%d\n", chg->sw_cv_dynamic_fcc);
		} else {
			cancel_delayed_work(&chg->sw_cv_fcc_limit_work);
			return;
		}
	}
	schedule_delayed_work(&chg->sw_cv_fcc_limit_work,
...
#ifdef CONFIG_BQ2597X_CHARGE_PUMP
	if (is_client_vote_enabled(chg->fcc_votable, MAIN_FCC_VOTER)) {
		vote(chg->fcc_votable, MAIN_FCC_VOTER, false, 0);
		cancel_delayed_work(&chg->sw_cv_fcc_limit_work);
	}
#endif
...
#ifdef CONFIG_BQ2597X_CHARGE_PUMP
	INIT_DELAYED_WORK(&chg->sw_cv_fcc_limit_work, sw_cv_fcc_limit_work);
